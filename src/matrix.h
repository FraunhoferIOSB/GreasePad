...
public:
    ...
    SparseMatrix<int> biadjacency() const;   // [O, A; A', O]

    void   set( Index r, Index c)  { coeffRef(r,c) = 1; }
    void unset( Index r, Index c)  { coeffRef(r,c) = 0; }

    void remove_row(    int r );
    void remove_column( int c );
    void reduce( int i);
    void augment() { conservativeResize( this->rows()+1, this->cols()+1); }
